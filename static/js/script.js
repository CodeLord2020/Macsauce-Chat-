...
// Function to switch the color at a specified interval
function switchColor() {
  // Get the current value of --color-main
  const currentColor = getComputedStyle(root).getPropertyValue('--color-main').trim();
  
  // Find the index of the current color in the colorOptions array
  const currentIndex = colorOptions.indexOf(currentColor);
  
  // Calculate the index of the next color
  const nextIndex = (currentIndex + 1) % colorOptions.length;
  
  // Get the next color from the colorOptions array
  const nextColor = colorOptions[nextIndex];
  
  // Set the value of --color-main to the next color
